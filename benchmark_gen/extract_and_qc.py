...
def write_transliterations_csv():
    rows = []
    for txt_path in sorted(OUT_DIR.glob("*.txt")):
        if txt_path.name.endswith(".orig.txt"):
            continue
        jpg_path = txt_path.with_suffix(".jpg")
        if not jpg_path.exists():
            continue
        transcription = txt_path.read_text(encoding="utf-8").strip()
        rows.append({"image_file_name": jpg_path.name, "transcription": transcription})

    csv_path = OUT_DIR / "transliterations.csv"
    with csv_path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=["image_file_name", "transcription"])
        writer.writeheader()
