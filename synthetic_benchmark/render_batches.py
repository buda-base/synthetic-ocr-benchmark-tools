...
def write_benchmark_metadata(out_dir: Path, catalog_rows: list[dict[str, object]]) -> None:
    ...
    (dataset_root / "README.md").write_text(
        "# BEC Synthetic Tibetan OCR Benchmark\n\n"
        "This dataset is a synthetic Tibetan OCR benchmark created by the Buddhist Digital "
        "Resource Center (BDRC) for the BDRC Etext Corpus project. It pairs rendered "
        "Tibetan page images with page-level transcriptions derived from the "
        "[OpenPecha BoCorpus](https://huggingface.co/datasets/openpecha/BoCorpus). "
        "The font collection and benchmark generation code are maintained in the "
        "[OpenPecha Tibetan Fonts repository](https://github.com/openpecha/tibetan-fonts).\n\n"
        "## Construction\n\n"
        "Text was normalized and chunked from BoCorpus into page-sized Tibetan passages. "
        "Each chunk was tokenized into Tibetan stacks, and each stack was checked against "
        "font-specific support data produced with HarfBuzz shaping plus geometric placement "
        ...
        "produced for this benchmark are also public domain.\n\n"
        "## How To Cite\n\n"
        "Please cite the Buddhist Digital Resource Center, the BDRC Etext Corpus project, "
        "the OpenPecha BoCorpus, and the OpenPecha Tibetan Fonts repository when using "
        "this dataset.\n",
        encoding="utf-8",
    )
