...
- **top mark overlap** (`top_mark_overlap`): threshold 0.60, but only when the mark does not reach the preceding glyph's head line (so simple `tsho` naro-on-head passes). Motivating review case: AmdoClassic4 `tshuM`. Exception: ཙ / ཚ / ཛ / ྩ / ྪ / ྫ with a *single* top mark that still reaches the base head (tsa-phru tick nesting); mid-body marks (KhaWa `dz+dz+aM`) and multi-mark pile-ups (`tshoM`, `dzi+i`) still fail
...
- floating bottom-vowel geometry in tall stacks (requires the vowel barely protrude below the previous layer; normal āchung / zhabs-kyu attachment that overlaps but hangs below is allowed — Monlam Lakdi `dzaA` / `k+Sh+Nu`)
- **superscript merge** (`superscript_merge`): a separate ra-go glyph is mostly merged into the next letter's head instead of sitting above it. Motivating review case: Monlam_Lakdi_Ouchen `r+gb+h+yaA`. Exception: ྩ / ྪ / ྫ (and base ཙ / ཚ / ཛ) already include a tsa-phru tick, so nesting into that tick is allowed (`rts` / `rtsh` / `rdz` stacks)
